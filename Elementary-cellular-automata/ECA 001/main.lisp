;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; 
;; This file demonstrates the concept of elementary cellular automata
;; (ECA), a species of the cellular automaton realm which is restricted
;; to 8-bit patterns whence the separate lines of generations,
;; one-dimensional in their composition and binary in their states,
;; derive their evolution.
;; 
;; 
...
(deftype automaton-rule ()
  "The ``automaton-rule'' type defines an elementary cellular automaton
   rule as a composition of eight consecutive bits, thus representative
   of the integral range [0, 255], the same exhausts the 256 available
   rule specimens."
  '(unsigned-byte 8))
...
(deftype bit-key ()
  "The ``bit-key'' type defines a neighborhood ensconcing a bit-valued
   elementary cellular automaton cell and its immediate sinistral and
   dextral neighbor as a composition of three consecutive bits."
  '(unsigned-byte 3))
...
(defun apply-rule (rule bit-key)
  "Returns the output bit associated with the elementary cellular
   automaton RULE for a three-cells neighborhood established by the
   BIT-KEY."
  (declare (type automaton-rule rule))
  (declare (type bit-key        bit-key))
  (the bit
    (if (logbitp bit-key rule) 1 0)))

;;; -------------------------------------------------------

(defun get-line-bit-at (line index)
  "Returns the bit of the cell amenable to the INDEX in the LINE, or
   responds with the default value of zero (0) upon the valid bournes'
   transgression."
  (declare (type bit-vector line))
  (declare (type fixnum     index))
  (the bit
    (if (array-in-bounds-p line index)
      (bit line index)
      0)))

;;; -------------------------------------------------------

(defun build-bit-key (line center-index)
  "Builds and returns a bit key, a three-bit neighborhood around the
   cell located at the CENTER-INDEX on the binary LINE, the sinistral
   neighbor storing the most significant bit, the center cell the next
   lower one, and the dextral neighbor the least significant one, thus
   providing a key for an elementary cellular automaton rule."
  (declare (type bit-vector line))
  (declare (type fixnum     center-index))
  (let ((bit-key #b000))
    (declare (type bit-key bit-key))
    ;; Specify the left neighbor.
    ;; [!] Please do not lapse in here: The left neighbor corresponds
    ;;     to the left bit of the triad, and thus the most significant
    ;;     bit position (MSB), that is, in this case: the index 2.
    (setf (ldb (byte 1 2) bit-key)
      (get-line-bit-at line
        (1- center-index)))
    ;; Specify the cell itself.
    (setf (ldb (byte 1 1) bit-key)
      (get-line-bit-at line center-index))
    ;; Specify the right neighbor.
    ;; [!] Please do not lapse in here: The left neighbor corresponds
    ;;     to the right bit of the triad, and thus the least significant
    ;;     bit position (LSB), that is, in this case: the index 0.
    (setf (ldb (byte 1 0) bit-key)
      (get-line-bit-at line
        (1+ center-index)))
    (the bit-key bit-key)))
...
(defun print-line (line
                   &key (zero-bit-symbol #\Space)
                        (one-bit-symbol  #\#))
  "Prints the bit pattern LINE to the standard output, visualizing every
   zero-bit (0) as a ZERO-BIT-SYMBOL and every one-bit (1) as a
   ONE-BIT-SYMBOL, and returns no value."
  (declare (type bit-vector line))
  (declare (type character  zero-bit-symbol))
  (declare (type character  one-bit-symbol))
  (terpri)
  (loop for bit of-type bit across line do
    (format T "~a"
      (if (zerop bit)
        zero-bit-symbol
        one-bit-symbol)))
...
(defun visualize-automaton (rule width height
                            &key (zero-bit-symbol #\Space)
                                 (one-bit-symbol  #\#))
  "Accommodates a viewport of the WIDTH and HEIGHT, visualizes the
   elementary cellular automaton RULE's effect, naiting the
   ZERO-BIT-SYMBOL to format zero-valued bits and the ONE-BIT-SYMBOL for
   the one-bits, and returns no value."
  (declare (type automaton-rule rule))
  (declare (type fixnum         width))
  (declare (type fixnum         height))
  (declare (type character      zero-bit-symbol))
  (declare (type character      one-bit-symbol))
  ;; Every subsequent line printed represents a new generation.
  (let ((current-line (make-array width
                        :element-type    'bit
                        :initial-element 0
                        :adjustable      NIL
                        :fill-pointer    NIL))
        (next-line    (make-array width
                        :element-type    'bit
                        :initial-element 0
                        :adjustable      NIL
                        :fill-pointer    NIL)))
    (declare (type simple-bit-vector current-line))
    (declare (type simple-bit-vector next-line))
    
    ;; Set the center bit of the initial generation, represented by the
    ;; CURRENT-LINE, to 1.
    (setf (sbit current-line (floor width 2)) 1)
    
    ;; Print the CURRENT-LINE, which constitutes the initial generation.
    (print-line current-line
      :zero-bit-symbol zero-bit-symbol
      :one-bit-symbol  one-bit-symbol)
    
    (loop for row of-type fixnum from 0 below height do
      
      ;; Build the NEXT-LINE's bit pattern, tantamount to the next
      ;; generation.
      (loop for column of-type fixnum from 1 below (1- width) do
        (setf (sbit next-line column)
          (apply-rule rule
            (build-bit-key current-line column))))
      
      ;; Output the binary pattern created by the automaton rule.
...
(visualize-automaton 60 45 20)

;;; -------------------------------------------------------

(visualize-automaton 126 49 23)

;;; -------------------------------------------------------

(visualize-automaton 90 45 20 :zero-bit-symbol #\. :one-bit-symbol #\X)
